...
type Product = { id: string; name: string; description: string; price: number; ticketsTotal: number; status?: 'active' | 'closed'; raffleDate?: string; drawMode?: 'scheduled' | 'sell_out'; gallery?: { imageUrl?: string }[] }
...
export function ProductCard({ product }: { product: Product }) {
  ...
  return <article className="card">
    <div className="card-image">{displayImage && <img src={displayImage} alt={`Premio: ${product.name}`} />}<span>{product.status === 'closed' ? 'RIFA FINALIZADA' : 'RIFA ACTIVA'}</span></div>
